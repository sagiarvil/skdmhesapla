#!/usr/bin/env node
/**
 * Sitemap üretimi — sitemap_1.txt + docs/seo-haritasi.md
 * lastmod = MIN(git içerik zamanı, şimdi). Build/deploy saati YASAK.
 * XML: yalnız loc + lastmod. priority/changefreq YASAK.
 */
...
/** W3C Datetime UTC. Kanıt yoksa null (etiket omit). Gelecek tarih kırpılır. */
...
export function sitemapEntries(config, registry, now = new Date()) {
  return registry.entries
    ...
    .map((e) => ({
      ...
      route: e.route,
      lastmod: lastmodForRoute(e.route, now),
    }))
    .sort((a, b) => a.loc.localeCompare(b.loc, "en"));
}

export function buildSitemapXml(entries) {
  if (entries.length > CHUNK_MAX) {
    throw new Error(`sitemap chunk rollover: ${entries.length} > ${CHUNK_MAX} — index'e bölünmeli, üretim durmaz`);
  }
  const body = entries
    .map((e) => {
...
export function writeSitemapAtomic(xml) {
  if (!xml.includes("<url>")) {
    throw new Error("S19: 0 URL — canlı sitemap ezilmez");
  }
...
export function assertSitemapQuality(xml, entries, baseline) {
  ...
  if (baseline?.urlCount) {
    ...
    if (drop > 0.2 && process.env.SEO_SITEMAP_FORCE !== "1") {
      errors.push(`URL düşüşü ${(drop * 100).toFixed(0)}% >20% — SEO_SITEMAP_FORCE=1 olmadan yazılmaz`);
    }
  }
...
export function writeBaseline(xml, entries) {
  ...
  const payload = {
    ...
    status,
    urls: Object.fromEntries(entries.map((e) => [e.loc, e.lastmod])),
  };
