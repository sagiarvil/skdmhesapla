...
exports.resolvePcfFactor = resolvePcfFactor;
const factor_registry_1 = require("./factor-registry");
function supplierFactorToResolved(req, sf) {
...
function geographyRank(f, geography) {
    if (f.geographies.includes(geography))
        return 30;
    if (f.geographies.includes("GLOBAL"))
        return 20;
    if (f.geographies.includes("GLOBAL_PROXY"))
        return 10;
    return 0;
}
function connectionMatches(f, connectionType) {
    if (f.kind !== "electricity")
        return true;
    if (connectionType !== "distribution" && connectionType !== "transmission")
        return false;
    if (f.connectionType)
        return f.connectionType === connectionType;
...
function resolvePcfFactor(request, registry) {
    if (request.supplierFactor)
        return supplierFactorToResolved(request, request.supplierFactor);
    if (request.kind === "electricity" &&
        request.connectionType !== "distribution" &&
        request.connectionType !== "transmission") {
        return {
            ok: false,
            code: "PCF_ELECTRICITY_CONNECTION_UNKNOWN",
            messageTr: "Elektrik bağlantı tipi belirtilmeden şebeke faktörü seçilmez; dağıtım hattı varsayılmadı.",
        };
    }
    const candidates = registry.filter((f) => {
        if (f.kind !== request.kind)
            return false;
        ...
        if (!direct && !proxy)
            return false;
        if (!(0, factor_registry_1.isReviewStatusUsable)(f.reviewStatus, request.allowEstimate))
            return false;
        if (!(0, factor_registry_1.isFactorValidAt)(f, request.asOfIso))
            return false;
        if (!connectionMatches(f, request.connectionType))
            return false;
        if (geographyRank(f, request.geography) === 0)
            return false;
        return true;
    });
    candidates.sort((a, b) => {
