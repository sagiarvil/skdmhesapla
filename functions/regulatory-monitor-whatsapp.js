...
const USER_AGENT = "SKDMHesapla-Regulatory-Watch/1.1 (+https://skdmhesapla.com)";
const DEFAULT_WHATSAPP_TO = "905393333303";
...
function meaningfulTokens(text) {
  return new Set(String(text || "").toLowerCase().split(/[^\p{L}\p{N}./%-]+/u).filter((x) => x.length >= 3).slice(0, 30000));
}
...
function buildDiff(previousText, currentText) {
  ...
  const added = [];
  const removed = [];
  for (const token of after) { if (!before.has(token)) added.push(token); if (added.length >= 220) break; }
  for (const token of before) { if (!after.has(token)) removed.push(token); if (removed.length >= 220) break; }
...
function classifyChange(target, diffText) {
  ...
  const calculation = has("default value", "default values", "benchmark", "embedded emission", "emission factor", "methodology", "calculation", "certificate price", "annex iv", "cn code", "combined nomenclature");
  const legal = target.authority === "EUR_LEX" || has("regulation", "implementing regulation", "delegated regulation", "official journal", "entry into force", "shall apply");
  ...
  return { severity, calculation, legal, deadline, registry, verification, requiresHumanReview: severity === "P0" || severity === "P1", autoDeployAllowed: false };
}
...
async function fetchOfficial(url) {
  const response = await fetch(url, {
    method: "GET",
    redirect: "follow",
    headers: { "user-agent": USER_AGENT, accept: "text/html,application/xhtml+xml,application/xml;q=0.9,text/plain;q=0.8,*/*;q=0.5" },
  ...
  return { normalized, hash: sha256(normalized), etag: response.headers.get("etag") || null, lastModified: response.headers.get("last-modified") || null };
}
...
async function sendWhatsApp(event) {
  ...
  const recipient = env("REG_WHATSAPP_TO", DEFAULT_WHATSAPP_TO).replace(/^\+/, "");
  ...
  if (!token || !phoneNumberId || !recipient) return { channel: "whatsapp", status: "skipped", reason: "not_configured" };
  ...
  if (!response.ok) throw new Error(`whatsapp HTTP ${response.status}: ${(await response.text()).slice(0, 600)}`);
...
async function dispatchImmediate(event) {
  ...
  const results = [];
  try { results.push(await sendWhatsApp(event)); } catch (error) { results.push({ channel: "whatsapp", status: "failed", reason: String(error.message || error).slice(0, 500) }); }
  try { results.push(await sendEmail(`[${event.severity}] CBAM resmî güncelleme`, text)); } catch (error) { results.push({ channel: "email", status: "failed", reason: String(error.message || error).slice(0, 500) }); }
  return results;
}
...
async function processTarget(target) {
  ...
  try {
    ...
    if (!previous || !previous.hash) {
      await ref.set({ ...target, hash: current.hash, normalizedText: current.normalized, etag: current.etag, lastModified: current.lastModified, status: "baseline", firstSeenAt: fetchedAt, lastCheckedAt: fetchedAt, consecutiveFailures: 0 }, { merge: true });
    ...
    const event = {
      eventId, targetId: target.id, authority: target.authority, authorityType: target.type, sourceUrl: target.url,
      previousHash: previous.hash, currentHash: current.hash, detectedAt: fetchedAt, severity: impact.severity, impact, diff,
      status: impact.requiresHumanReview ? "REVIEW_REQUIRED" : "DETECTED",
      publishStatus: "BLOCKED_PENDING_REVIEW", calculationDeployStatus: "BLOCKED", autoDeployAllowed: false,
    };
    if (!eventSnap.exists) {
    ...
    await ref.set({ ...target, hash: current.hash, normalizedText: current.normalized, etag: current.etag, lastModified: current.lastModified, status: "changed", lastChangedAt: fetchedAt, lastCheckedAt: fetchedAt, consecutiveFailures: 0 }, { merge: true });
    ...
    await ref.set({ ...target, status: "fetch_failed", lastCheckedAt: fetchedAt, lastFailureAt: fetchedAt, lastFailure: String(error.message || error).slice(0, 800), consecutiveFailures: failures }, { merge: true });
    if (failures >= 2) {
      ...
      if (!healthSnap.exists) {
        const event = {
          eventId: healthId, targetId: target.id, authority: target.authority, authorityType: "SOURCE_HEALTH", sourceUrl: target.url,
          detectedAt: fetchedAt, severity: "P1", impact: { sourceHealth: true, requiresHumanReview: true, autoDeployAllowed: false },
          diff: `Kaynak ${failures} ardışık kontrolde okunamadı.`, status: "REVIEW_REQUIRED", publishStatus: "NOT_APPLICABLE",
          calculationDeployStatus: "BLOCKED", autoDeployAllowed: false,
        };
...
async function runWatch() {
  ...
  await db.collection("regulatory_runs").add({ startedBy: "scheduler", finishedAt: nowIso(), results, sourceCount: OFFICIAL_TARGETS.length, watcherVersion: "reg-watch-v1.1.0-whatsapp" });
  return results;
}
...
exports.regulatoryWatch15m = onSchedule({
  ...
  retryCount: 1,
  maxInstances: 1,
  secrets: [whatsappAccessToken, resendApiKey],
}, async () => { await runWatch(); });
...
exports.regulatoryDigestDaily = onSchedule({
  ...
  const events = snap.docs.map((doc) => doc.data()).filter((event) => event.severity === "P2" || event.severity === "P3");
  if (!events.length) return;
  const body = [`CBAM günlük resmî kaynak özeti (${events.length} olay)`, "", ...events.map((event) => `${event.severity} | ${event.authority} | ${event.targetId}\n${event.sourceUrl}`)].join("\n\n");
  const results = [];
  ...
  await db.collection("regulatory_digest_runs").add({ sentAt: nowIso(), eventCount: events.length, results, source: "daily_digest", createdAt: FieldValue.serverTimestamp() });
});
