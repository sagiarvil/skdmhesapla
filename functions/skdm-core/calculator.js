...
exports.calculateSkdmLiability = calculateSkdmLiability;
exports.deMinimisVerdictFor = deMinimisVerdictFor;
const config_1 = require("./config");
const audit_1 = require("./audit");
const fuel_emission_factors_1 = require("./fuel-emission-factors");
function calculateSkdmLiability(input) {
    const sector = config_1.SKDM_SECTORS[input.sectorId] || config_1.SKDM_SECTORS["iron-steel"];
    ...
    const etsQuarter = input.etsQuarter || config_1.DEFAULT_ETS_QUARTER;
    const rulesetEtsPrice = config_1.etsPriceQuarterly[etsQuarter] ?? config_1.DEFAULT_EU_ETS_PRICE_EUR;
    ...
    const trEtsNettingEur = (0, config_1.resolveTrEtsNettingEur)(year, input.trEtsNettingEur);
    const effectiveCarbonPriceEur = Math.max(0, euEtsPriceEur - trEtsNettingEur);
    const eurTryRate = input.eurTryRate ?? config_1.DEFAULT_EUR_TRY_RATE;
    ...
    const freeAllocationRatio = config_1.CBAM_DECAY_SCHEDULE[year] ?? 0.0;
    ...
    const streamResults = (input.streams || [])
        .map((s) => ({ s, r: (0, fuel_emission_factors_1.resolveStreamEmission)(s) }))
        .filter((x) => x.r !== null);
    const nonElectricResults = streamResults.filter(({ s }) => !(0, fuel_emission_factors_1.isElectricityStream)(s));
    const electricResults = streamResults.filter(({ s }) => (0, fuel_emission_factors_1.isElectricityStream)(s));
    ...
    if (nonElectricResults.length > 0) {
        for (const { s, r } of nonElectricResults) {
            emissionSteps.push({
                kind: (0, fuel_emission_factors_1.isDirectEmissionStream)(s) ? "process" : "combustion",
                label: s.name,
                formula: r.formula,
    ...
    const derivedStepCount = nonElectricResults.length +
        (scope2Applicable
            ? electricResults.filter(({ r }) => r.dataClass !== "default-electricity-grid").length
            : 0);
    const emissionDataQuality = derivedStepCount > 0
        ? "dogrudan-olcum"
        : "varsayilan-deger";
    ...
    const quarterlyHoldingEmissions = liableEmissions * config_1.CBAM_QUARTERLY_HOLDING_RATIO;
    const quarterlyHoldingCostEur = importerCostEur * config_1.CBAM_QUARTERLY_HOLDING_RATIO;
    ...
    const audit = (0, audit_1.generateSkdmAuditHash)({
        sectorId: sector.id,
        year,
 ...
 */
function deMinimisVerdictFor(result) {
    ...
    if (result.importerAnnualVolumeStatus === "under50") {
        return {
            ...
            detail: "MUAF — alıcınızın (AB ithalatçısının) takvim yılı içindeki toplam SKDM kapsamı ithalatı 50 tonun altında; sertifika maliyeti 0 EUR.",
        };
    }
    if (result.importerAnnualVolumeStatus === "over50") {
        return {
            ...
            detail: "TABİ — alıcınızın yıllık toplam SKDM ithalatı 50 tonun üzerinde; normal SKDM maliyetlendirmesi uygulanır.",
        };
    }
    return {
        ...
        detail: "Belirlenemedi — bu sistemde hüküm üretilmez. Alıcınızın yıllık toplam ithalatı teyit edilmeden MUAF/TABİ kararı verilmemelidir.",
    };
}
