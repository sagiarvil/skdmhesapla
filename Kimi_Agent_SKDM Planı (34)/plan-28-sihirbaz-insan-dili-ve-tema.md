...
## EK (v3) — Ekran görüntüsü geri bildirimi sonrası sadelik turu

Kullanıcı geri bildirimi ("bu yapı kullanıcı dostu değil") üzerine sihirbaz Claude'un prototipine
birebir yaklaştırıldı:

- **Sağ panel tamamen kaldırıldı.** "Şu anda eksik olanlar" listesi artık her adımda ekranı
  bölmüyor; yalnızca 10. adımda (özet ve mühür) "Mühürlemeden önce şunlar tamamlanmalı" bloğu
  olarak görünüyor.
- **`B_STREAM_MISSING` gibi ham kodlar temizlendi** — eksik listesi artık insan dili mesajları
  gösteriyor.
- **Doluluk skoru kartı kaldırıldı.** Boş dosyada "%18" görünmesi sorunu kökten çözüldü:
  prototipte olduğu gibi skor hiç gösterilmiyor; ilerleme yalnızca ince adım şeridinde.
- **Tek sütun, ortalanmış düzen** (max 3xl) — prototiple aynı okuma ritmi.
- Boş dosyada yanlış skor üreten `uiScore` hesabı ve kullanılmayan import'lar kaldırıldı
  (build temizliği).

Uygulama aynı: dosyayı `src/components/wizard/SkdmWizard.tsx` üzerine kopyala → build → deploy → push.
