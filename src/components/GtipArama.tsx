  ...
  return (
    <div className="w-full">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <label
          htmlFor="gtip-arama"
          className="flex items-center gap-2 text-base font-extrabold text-ink-900 sm:text-lg"
        >
          <Sparkles className="h-5 w-5 text-brand-800" />
          <span>GTİP Kodunuzu veya Ürününüzü Yazın:</span>
        </label>
        <span className="rounded-pill bg-brand-800/10 px-3 py-1 text-xs font-black text-brand-900 border border-brand-800/20 uppercase tracking-wider">
          Canlı Eşleştirme Motoru
        </span>
      </div>

      {/* Ultra-Lüks Arama Çubuğu */}
      <div className="relative flex items-center rounded-2xl border-2 border-brand-800/30 bg-white p-2 shadow-2xl transition-all focus-within:border-brand-800 focus-within:ring-4 focus-within:ring-brand-500/25">
        ...
        <input
          ...
          className="w-full bg-transparent px-3 py-2 text-lg font-bold text-ink-900 outline-none placeholder:font-normal placeholder:text-ink-600/70 sm:text-xl"
        />
      ...
      <p className="mt-2.5 text-xs font-semibold text-ink-600 leading-relaxed sm:text-sm">
        💡 Önerilen kod bilgilendirme amaçlıdır; kesin GTİP teyidini gümrük beyannamenizle yapınız.
      </p>
    </div>
  );
}
