...
export function EuBuyerLeadForm() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    setSubmitted(true);
    track("eu_importer_form_success", { role, supplierCount, sector });
      ...
      {submitted ? (
        <div className="mt-8 space-y-6 rounded-2xl border-2 border-emerald-300 bg-gradient-to-br from-emerald-50/90 via-white to-emerald-50/50 p-6 sm:p-8">
          <div className="flex items-start gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-sm leading-relaxed text-ink-700">
                Your collection requirements have been formatted and your email client has opened. You can also copy the structured inquiry below to send directly to <strong>{targetEmail}</strong>.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <a
              href={`mailto:${targetEmail}?subject=${encodeURIComponent(`[CBAM Supplier Collection] ${companyName}`)}&body=${encodeURIComponent(getSummaryText())}`}
        ...
        <form onSubmit={handleSubmit} onFocus={handleInteraction} className="mt-6 space-y-5">
          ...
          >
            <Send className="h-4 w-4" /> Submit Collection Inquiry
          </button>
        </form>
      )}
    </div>
