...
const roles: { value: MaritimeRole; label: string }[] = [
  { value: "gemi-sahibi", label: "Registered owner / gemi sahibi" },
...
const shipTypes: { value: MaritimeShipType; label: string }[] = [
  ...
  { value: "general-cargo", label: "General cargo ship" },
  { value: "passenger", label: "Passenger ship" },
  { value: "offshore", label: "Offshore ship" },
  { value: "other", label: "Diğer / kategori net değil" },
];
const portRegions: { value: MaritimePortRegion; label: string }[] = [
  { value: "eu", label: "AB liman bağlantısı var" },
  { value: "norway-iceland", label: "Norveç / İzlanda bağlantısı var" },
  { value: "none", label: "AB/AEA liman bağlantısı yok" },
  { value: "unknown", label: "Henüz net değil" },
];
...
export function MaritimeScopeWizard() {
  ...
  return (
    <section className="pasaport-zemin-acik border-b border-line bg-bg-soft py-12 sm:py-16">
      <div className="mx-auto max-w-6xl px-5 sm:px-6">
        <div className="mb-8 max-w-3xl">
          ...
          <p className="mt-4 text-base font-semibold leading-7 text-ink-700">Kural seti {MARITIME_RULESET_REVIEWED_AT} tarihinde resmî EU MRV, EU ETS ve FuelEU kaynaklarına göre gözden geçirilmiştir. Sonuç, tam dosya akışındaki gerekli alanları otomatik açar.</p>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[1.03fr_.97fr]">
          <div className="rounded-3xl border border-line bg-white p-6 shadow-xl sm:p-8">
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              <label className="text-sm font-black">Firma rolü<select value={role} onChange={(e) => setRole(e.target.value as MaritimeRole)} className="mt-2 min-h-12 w-full rounded-xl border border-line bg-white px-4 text-sm font-bold">{roles.map((i) => <option key={i.value} value={i.value}>{i.label}</option>)}</select></label>
              <label className="text-sm font-black">Gemi kategorisi<select value={shipType} onChange={(e) => setShipType(e.target.value as MaritimeShipType)} className="mt-2 min-h-12 w-full rounded-xl border border-line bg-white px-4 text-sm font-bold">{shipTypes.map((i) => <option key={i.value} value={i.value}>{i.label}</option>)}</select></label>
              ...
              <label className="text-sm font-black">Liman / rota bağlantısı<select value={portRegion} onChange={(e) => setPortRegion(e.target.value as MaritimePortRegion)} className="mt-2 min-h-12 w-full rounded-xl border border-line bg-white px-4 text-sm font-bold">{portRegions.map((i) => <option key={i.value} value={i.value}>{i.label}</option>)}</select></label>
              ...
              <label className="text-sm font-black">Raporlama yılı<select value={emissionsYear} onChange={(e) => setEmissionsYear(Number(e.target.value))} className="mt-2 min-h-12 w-full rounded-xl border border-line bg-white px-4 text-sm font-bold">{[2024, 2025, 2026, 2027, 2028, 2029, 2030].map((y) => <option key={y}>{y}</option>)}</select></label>
            </div>

            {role !== "gemi-sahibi" && <label className="mt-5 flex items-start gap-3 rounded-2xl border border-accent-yellow/40 bg-accent-yellow/10 p-4 text-sm font-bold text-ink-900"><input type="checkbox" checked={hasFormalMandate} onChange={(e) => setHasFormalMandate(e.target.checked)} className="mt-1 h-4 w-4" /><span><b>Formal responsibility mandate var.</b><br/><span className="font-semibold text-ink-700">Registered owner dışındaki şirketin ISM sorumluluğunu üstlendiğini ve MRV/ETS sorumluluğunun devrini gösteren belge referansı tam dosyada istenir.</span></span></label>}

            <div className="mt-5 grid gap-3">
              {[["Yakıt / BDN kayıtları hazır", hasFuelRecords, setHasFuelRecords], ["Sefer / port call kayıtları hazır", hasVoyageRecords, setHasVoyageRecords], ["Monitoring Plan mevcut", hasMonitoringPlan, setHasMonitoringPlan], ["CBAM kapsamlı ihracatçı müşterisi/yükü var", carriesCbamGoods, setCarriesCbamGoods]].map(([label, checked, setter]) => <label key={String(label)} className="flex items-center gap-3 rounded-xl border border-line px-4 py-3 text-sm font-bold"><input type="checkbox" checked={Boolean(checked)} onChange={(e) => (setter as (v: boolean) => void)(e.target.checked)} className="h-4 w-4" />{label as string}</label>)}
            </div>

            <div className="mt-6 rounded-2xl border border-brand-800/15 bg-brand-100/40 p-4"><div className="flex items-center gap-2 text-sm font-black"><Calculator className="h-4 w-4" /> Opsiyonel EU ETS maliyet ön tahmini</div><p className="mt-1 text-xs font-semibold leading-5 text-ink-600">ETS coğrafi kapsamına girdikten sonraki tCO₂e ve seçtiğiniz EUA fiyatını girin. Canlı piyasa fiyatı otomatik varsayılmaz.</p><div className="mt-3 grid gap-3 sm:grid-cols-2"><input inputMode="decimal" value={etsEmissions} onChange={(e) => setEtsEmissions(e.target.value)} placeholder="ETS kapsam tCO₂e" className="min-h-12 rounded-xl border border-line bg-white px-3 text-sm font-bold" /><input inputMode="decimal" value={euaPrice} onChange={(e) => setEuaPrice(e.target.value)} placeholder="EUA EUR/t" className="min-h-12 rounded-xl border border-line bg-white px-3 text-sm font-bold" /></div></div>
          </div>
          ...
          <aside className="rounded-3xl border border-brand-500/20 bg-brand-900 p-6 text-white shadow-xl sm:p-8">
            ...
            <div className="mt-5"><div className="flex justify-between text-sm font-black"><span>İlk veri hazırlığı</span><span>%{result.readinessScore}</span></div><div className="mt-2 h-3 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-brand-500" style={{ width: `${result.readinessScore}%` }} /></div></div></div>
            ...
            {result.warnings.map((w) => <div key={w} className="mt-3 flex gap-2 rounded-xl border border-accent-yellow/30 bg-accent-yellow/10 p-3 text-xs font-semibold leading-5 text-white"><CircleHelp className="mt-0.5 h-4 w-4 shrink-0" />{w}</div>)}
