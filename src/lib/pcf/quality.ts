...
export function summarizePcfQuality(
  contributions: readonly PcfContribution[],
  evidence: PcfEvidenceFlags,
  unresolvedCount: number,
): { readonly status: PcfReportStatus; readonly quality: PcfQualitySummary; readonly findings: readonly PcfFinding[] } {
  const findings: PcfFinding[] = [];
  ...
  const nonBuyerReadyFactors = contributions.filter(
    (c) =>
      !c.factor.buyerReadyEligible ||
      !PCF_POLICY.buyerReadyAllowedReviewStatuses.has(c.factor.reviewStatus) ||
      !PCF_POLICY.buyerReadyAllowedFactorQualities.has(c.factor.quality),
  );
  ...
  const status: PcfReportStatus = blocking ? "blocked" : buyerReady ? "buyer_ready" : "estimated";

  let grade: PcfQualitySummary["grade"] = "DQ4";
  if (buyerReady && verifiedSupplierContributionRatio >= 0.5) grade = "DQ1";
  else if (buyerReady) grade = "DQ2";
  else if (!blocking && proxyContributionRatio === 0) grade = "DQ3";
