/**
 * Mühür yetkisi — istemci amount/readiness/packageType güvenilmez.
 * Webhook-doğrulanmış skdm_orders kaydı olmadan seal yok.
 */

export type SealPackageType = "CBAM_SEAL_PACKAGE_9900" | "PCF_SEAL_PACKAGE_9900";
export type SealWorkflowType = "cbam" | "pcf";
...
export type OrderRecord = {
  ...
  currency?: string;
  consumedByPackageId?: string | null;
  consumedPackageType?: string | null;
};
...
export type EntitlementOk = { ok: true; entitlement: "cbam_seal" | "pcf_seal"; normalizedPackageType: SealPackageType };
export type EntitlementFail = { ok: false; http: 400 | 403; reason: string };
export type EntitlementResult = EntitlementOk | EntitlementFail;

export type PaymentStatusResponse =
  | { status: "pending" }
  | { status: "completed"; entitlement: "pcf_seal" | "cbam_seal" }
  | { status: "rejected"; reason: string };

export function normalizePackageType(raw: string | undefined): SealPackageType | null {
...
export function evaluateSealEntitlement(order: OrderRecord | null, req: SealRequest): EntitlementResult {
  if (!req.paddleTransactionId.trim() || !req.sessionId.trim() || !req.packageId.trim()) {
  ...
  const expectedWorkflow: SealWorkflowType = orderType === "PCF_SEAL_PACKAGE_9900" ? "pcf" : "cbam";
  ...
  if (req.workflowType === "cbam" && req.readinessScore !== 100) {
    return { ok: false, http: 403, reason: "Hazırlık skoru %100 olmadan mühür kaydı açılamaz" };
  }
  if (
    order.consumedByPackageId &&
    order.consumedByPackageId !== req.packageId
  ) {
    return { ok: false, http: 403, reason: "Bu ödeme başka bir paket için kullanılmış" };
  }
  if (
    order.consumedPackageType &&
    normalizePackageType(order.consumedPackageType) !== reqType
  ) {
    return { ok: false, http: 403, reason: "Bu ödeme başka bir paket için kullanılmış" };
  }
  return {
