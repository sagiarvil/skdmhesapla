...
export function isLegalEntityTitle(title: string | null | undefined): boolean {
  return TUREL_KISILIK_IBARESI.test(String(title || "").trim());
}

/** VKN kontrol hanesi — GİB algoritması (ilk 9 haneden 10. hane türetilir). */
export function computeVknCheckDigit(first9: string): number | null {
...
export interface TaxIdDenetim {
  durum: TaxIdDurum;
  ok: boolean;
  /** G-23 onaylı bulgu metni (qc katmanına bırakılır). */
  mesaj?: string;
}
