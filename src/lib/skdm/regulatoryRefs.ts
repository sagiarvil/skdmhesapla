...
export type RefUsage = { kind: "site" | "document"; name: string; section?: string };

export type RegulatoryRef = {
  id: string;
  /** Ekranda/PDF'te gösterilen kısa kod — tek kaynak. */
  code: string;
  fullTitleTr: string;
  fullTitleEn: string;
  topicTr: string;
...
export const REGULATORY_REFS: readonly RegulatoryRef[] = [
  {
    id: "cbam-2023-956",
    code: "AB 2023/956",
...
export const REG_REF: Record<string, string> = Object.fromEntries(
  REGULATORY_REFS.map((r) => [r.id, r.code])
);

export function regRefById(id: string): RegulatoryRef | undefined {
  return REGULATORY_REFS.find((r) => r.id === id);
}
