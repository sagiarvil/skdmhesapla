    ...
    [
      {
        ...
        lines: [
          ...
          spacer(5),
          note(
            `Mutabakat (GATE-A): Satır toplamı = ${result.emissionSteps.reduce((a, s) => a + s.emissions, 0).toFixed(2)} tCO2e, beyan edilen toplam ${result.totalEmissions.toFixed(2)} tCO2e ile kuruşu kuruşuna eşittir.`
          ),
          note(
...
${headerFooterText}`;
  ...
  const goodRows = (reg.goods || [])
    .map(
      (g, i) =>
        `G${i + 1},Aggregated Goods / Mal Kategorisi | CN | Route / Rota,${(g.category || "-").replace(/,/g, ";")} | CN ${(g.cn || "-").replace(/,/g, " ")} | ${(g.route || "-").replace(/,/g, ";")},-`
    )
    .join("\n");
  const processRows = (reg.processes || [])
    .map(
      (p, i) =>
        `P${i + 1},Production Process / Uretim Sureci,${(p.name || "-").replace(/,/g, ";")} | included/dahil=${(p.included || []).join("+")},-`
    )
    .join("\n");
  const streamRows = (reg.streams || [])
    .map(
      (s, i) =>
        `B${i + 1},Source Stream / Kaynak Akisi,${(s.method || "-")}|${(s.name || "-").replace(/,/g, ";")}|AD=${s.ad} ${s.unit}|P=${s.processId || "-"},-`
    )
    .join("\n");
  const precRows = (reg.precs || [])
    .map(
      (p, i) =>
        `E${i + 1},Precursor / Oncul Madde,${(p.name || "-").replace(/,/g, ";")}|total/toplam=${p.total}|SEE=${p.see},-`
    )
    .join("\n");
  ...
  const pdf8 = formalReportPdfBytes(
    ...
    [
      ...
      },
      {
        ...
        lines: [
          body(`İzleme planı formatı ve emisyon raporu yapısı: ${REG_REF["ir-2025-2547"]} (kesin dönem izleme ve raporlama kuralları / definitive period monitoring rules).`),
          body(`İzleme planı yükümlülüğü: ${REG_REF["cbam-2023-956"]}, Madde 8 (Article 8).`),
          body(`Varsayılan değer dayanağı: ${REG_REF["ir-2025-2621"]} (mark-up'lı varsayılan gömülü emisyon değerleri / marked-up default values).`),
        ],
      },
    ],
    pdfFooter
