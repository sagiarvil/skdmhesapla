...
function etsDecision(input: MaritimeScopeInput): { level: MaritimeScopeLevel; reason: string } {
  ...
  if (input.shipType === "other") return { level: "review", reason: "Özel gemi sınıfında ETS istisnaları manuel doğrulanmalıdır." };
...
function fueleuDecision(input: MaritimeScopeInput): { level: MaritimeScopeLevel; reason: string } {
  ...
  if (input.grossTonnage === 5000 || input.shipType === "other") {
    return { level: "review", reason: "5.000 GT sınırı veya gemi sınıfı nedeniyle hukuki kapsam manuel doğrulanmalıdır." };
  }
...
export function assessMaritimeScope(input: MaritimeScopeInput): MaritimeScopeResult {
  ...
  const headline = commercialRoute === "partner-desk"
    ...
    : commercialRoute === "annual-compliance"
      ...
      : commercialRoute === "paid-pre-analysis"
        ? "Kapsam kararı için gemi, rota veya sorumluluk zincirinde manuel kontrol gerekiyor."
  ...
  if (estimatedEtsCostEur !== null) warnings.push("ETS maliyeti canlı piyasa fiyatı değildir; kullanıcının girdiği ETS kapsam CO2e ve EUA fiyatı üzerinden ön tahmindir.");
...
export function levelLabel(value: MaritimeScopeLevel) {
  switch (value) {
    case "critical": return "Muhtemel kapsam";
