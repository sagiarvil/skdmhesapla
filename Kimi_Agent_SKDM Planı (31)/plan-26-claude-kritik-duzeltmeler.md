...
## EK (v4.1) — Kullanıcı ekran görüntüsü düzeltmeleri

1. **Arama kutusu iç çerçevesi kaldırıldı.** Safari/Chrome'un input'a çizdiği iç çizgi hem ana sayfa
   hem /basla/ aramasında görünüyordu. `GtipArama.tsx` ve `IcerikArama.tsx`'teki input'lara
   `appearance-none border-0 shadow-none ring-0 focus:ring-0 focus-visible:ring-0` + satır içi
   `style={{ outline: "none", boxShadow: "none", border: "none" }}` eklendi — hiçbir tarayıcıda
   iç çerçeve çizilemez.
2. **Hero başlığı düzeltildi:** "AB'ye satıyorsanız, karbon dosyanız hazır olmalı." →
   **"AB'ye ihracat yapıyorsanız, SKDM (CBAM) dosyanız hazır olmalı."**
3. **360° metin denetimi:** tüm CTA ve metin alanları kurumsal ama doğal Türkçeye çekildi;
   LLM kokan kalıplar ("Bir not dürüstlük adına", "10 kat daha ekonomik", abartılı "ultra-lüks"
   yorumları) temizlendi. Değişen dosyalar: `yeni-ana-sayfa-v4.tsx`, `yeni-GtipArama.tsx`,
   `yeni-IcerikArama.tsx`, `yeni-fiyatlandirma-page.tsx`.

| Verilen dosya | Repodaki hedef |
|---|---|
| `yeni-IcerikArama.tsx` | `src/components/IcerikArama.tsx` (tam değiştir) |
