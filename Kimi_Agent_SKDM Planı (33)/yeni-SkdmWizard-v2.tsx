...
export function SkdmWizard({ sectorSlug }: { sectorSlug: string }) {
  ...
  return (
    ...
    >
      ...
      <header className="flex flex-wrap items-end justify-between gap-4 border-b pb-6" style={{ borderColor: T.line }}>
        <div>
        ...
        <div className="min-w-[200px] rounded-[12px] border bg-white p-3.5" style={{ borderColor: T.line }}>
          <div className="flex justify-between text-xs" style={{ color: T.mute }}>
            <span>Dosyanızın doluluk oranı</span>
            <span className="font-mono text-base font-semibold" style={{ color: T.oliveDeep }}>
          ...
          <div className="mt-2 h-1.5 overflow-hidden rounded-full" style={{ background: T.line }}>
            <div
              ...
              style={{ width: `${hasRealInput ? uiScore : 0}%`, background: T.olive }}
            />
          </div>
          {!hasRealInput && (
            <p className="mt-1.5 text-[11px]" style={{ color: T.mute }}>
              Veri girdikçe burada ilerlemeyi göreceksiniz.
            </p>
          )}
        </div>
      </header>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_280px]">
        ...
        <aside className="space-y-4">
          <div className="rounded-[12px] border bg-white p-4" style={{ borderColor: T.line }}>
            <b className="text-sm" style={{ color: T.ink }}>Şu anda eksik olanlar</b>
            <ul className="mt-3 space-y-2">
              {missing.length === 0 ? (
                <li className="text-xs" style={{ color: T.inkSoft }}>Gerekli alanlar tamam görünüyor.</li>
              ) : (
                missing.map((m, i) => (
                  <li
                    key={m.name}
                    className="flex flex-col gap-1.5 rounded-[9px] px-3 py-2.5 text-xs"
                  ...
                  >
                    ...
                    <button
                      type="button"
                      className="self-start rounded-[8px] bg-white px-2.5 py-1.5 text-[11px] font-bold"
                      style={{ border: `1px solid ${T.line}`, color: T.oliveDeep }}
                      onClick={async () => {
                        if (m.copy) await navigator.clipboard.writeText(m.copy);
                      }}
                    >
                      {m.copy ? "Talep metnini kopyala" : m.action}
                    </button>
                  </li>
                ))
              )}
          ...
          <div className="rounded-[12px] border p-4 text-xs" style={{ borderColor: T.line, background: T.oliveWash, color: T.oliveDeep }}>
            <b>Neredesiniz?</b>
            <p className="mt-2 leading-relaxed">
              Adım {step}/10 · {goods.length} ürün, {processes.length} üretim adımı, {streams.length} enerji kaynağı, {precs.length} hammadde kaydettiniz.
            </p>
            <p className="mt-2 text-[11px]" style={{ color: T.inkSoft }}>
              Taslağınız otomatik kaydediliyor; tarayıcınızı kapatıp daha sonra kaldığınız yerden devam edebilirsiniz.
            </p>
          </div>
        </aside>
      </div>
