...
**İade politikası (Paddle uyumlu, sitede yayınlanır):** "Paket mühürlenip indirildikten sonra iade yoktur (anında ifa edilen dijital içerik); mühürleme öncesi tüm aşamalar ücretsizdir. Yeniden mühürleme 2.400 ₺ (KDV dahil) olup ücretsiz değildir. Teknik hata durumunda 14 gün içinde destek@skdmhesapla.com'a başvurulur."
...
Plan 16/17/18 klasörlerinin MD5 özdeş olduğu tespit edildi. Kural: (1) Plan numarası YALNIZCA içerik değişikliğinde artar; içerik aynıysa yeni plan numarası/klasörü açılmaz. (2) Tek kanonik aktif plan vardır: `docs/PLAN-AKTIF.md` her zaman gerçek içeriği gösterir; arşiv plan klasörleri salt okunur tarihçedir, işaretçi değişikliği "tur" sayılmaz. (3) Aktif plan numarası bu Ek içinde sabitlenmez — yalnız `docs/PLAN-AKTIF.md` okunur (Ek C yazıldığı anda işaret Plan 17 idi; içerik artışıyla Plan 19 aktif).

## Ek D — Plan 19 Genişletilmiş Kapsam (16.08.2026 hakem kararları)

1. **Kademe B/C listeleme (ONAY, §4.3A hibrit):** Ana sayfada Kademe A altına ayrı blok — başlık: «Zorunlu değil ama alıcınız isteyebilir». 14 sektör kartı. Çıktı asla «SKDM raporu» diye anılmaz; ISO 14067 mantığında **tedarikçi veri dosyası** çerçevesi. Kartlar `/rehber/#kademe-b` bağlantısına gider.

2. **Geri dön (ONAY, G-24 genişletme):** Sihirbazda ve tüm modül/içerik sayfalarında görünür «← Geri» (`router.back()`).

3. **CTA / 5 saniye testi (ONAY):** Ana sayfadan doğrudan hesaplamaya başlamak doğru. Hero Plan 14/13A kopyası **pazarlıksız**: rozet + H1 «AB'ye sattığınız ürünün karbon raporunu yardım almadan hazırlayın.» + «7 adımlık» + güven şeridi. UI kaynakta sekiz-adımlık ifade kalmamalı (`rg` ile `src/` temiz).

4. **İkonlar (ONAY):** cimpactpro tarzı Lucide çizgi ikonlar; emoji yok (yalnız ✓/!/✕ durum glifleri).

5. **Rehber + Sözlük:** `/rehber/` ve `/sozluk/` — içerik `docs/sayfa-icerik-rehber.md` + `docs/sayfa-icerik-sozluk.md`. Sözlük tanımları FieldHelp (?) panellerinin tek kaynağıdır (G-21).

6. **Tur sırası:** Tur 0 metin düzeltmeleri → Tur 1 register + Kademe B bloğu + rehber/sözlük + Lucide. Paddle 5 gerçek veri hâlâ kullanıcıda; Plan 20 = mühürlü ZIP.
