...
**Tek cümlelik konumlandırma:** *"CN kodunu seç, 7 adımlık sihirbazı tamamla (RM-SKDMHESAPLA-002 §2), SKDM maliyetini ve AB uyumlu emisyon raporunu dakikalar içinde al."*
...
### 4.3A Hakem Kararı — RELEASE MANDATE Entegrasyonu (RM-SKDMHESAPLA-001, 16 Ağustos 2026)

Antigravity ortamındaki Claude ajanının ürettiği RELEASE MANDATE incelendi ve **hibrit model** kararıyla entegre edildi:
...
Mandate'in tamamı proje `docs/PLAN-AKTIF.md` zincirinde kanonik; bu bölüm iki belge arasındaki çelişkileri çözen üst hakem kaydıdır. Çelişki durumunda: SKDM çekirdeğinde Mandate, genel ürün/pazarlama çerçevesinde bu Plan geçerlidir.
...
---
## Ek B — RM-004 Kabulü ve Öncelik Revizyonu (16.08.2026 hakem kararı)

**RM-004 (Veri Toplama UX + FieldHelp motoru) KANONİK KABUL:** Claude'un tek dosyalık tam sistem prototipi (10 katman + FIELD_DB + renderFieldHelp + Triyaj + "Şu anda eksik" paneli + yönetici özeti + kaldığım yerden devam) sihirbaz mimarisinin referans uygulamasıdır. Gerekçe: tek config nesnesinden 7 yardım unsuru üreten motor, ~45 alanı JSON girişiyle ölçekler — RM-002 G-21'in (terminoloji açıklama zorunluluğu) sistematik karşılığıdır.

**Koşullar:** (1) `--err:#B23B3B` kırmızısı G-23 ihlali → amber `#ffc21c` + "tamamlanmadı" dili; (2) görsel kaplama sitemizin token'larına eşlenir (prototip yeşil paleti uyumlu); (3) üretimde Next.js bileşenlerine taşınır, FIELD_DB → `src/lib/skdm/fieldhelp/fields.json`, kalıcılık Firestore `skdm_sessions` (G-22).

**Öncelik revizyonu (kullanıcı direktifi):** Fiyatlandırma/ödeme ve "son tamamlanacak" işler askıda. Tek odak: AB mevzuatına tam uyumlu, doğrulanabilirliği en güçlü, kurumsal kalitede veri toplama çekirdeği (G/P/B/E register'ları, kontrol denklikleri, FieldHelp). Paddle fiyat eşitlemesi ödeme açılmadan önce yapılacak 5 dk'lık ops işi olarak listede durur.

## Ek C — Plan Yönetişim Kuralı (16.08.2026 hakem kararı)

Plan 16/17/18 klasörlerinin MD5 özdeş olduğu tespit edildi. Kural: (1) Plan numarası YALNIZCA içerik değişikliğinde artar; içerik aynıysa yeni plan numarası/klasörü açılmaz. (2) Tek kanonik aktif plan vardır: `docs/PLAN-AKTIF.md` her zaman gerçek içeriği gösterir; arşiv plan klasörleri salt okunur tarihçedir, işaretçi değişikliği "tur" sayılmaz. (3) Şu anki kanonik aktif plan: **Plan 17** (son uygulanan işaret).

## Ek D — Kademe B Listeleme ve UX Kararları (16.08.2026)

1. **Kademe B ana sayfada listelenir:** 6 çekirdek sektör (Kademe A) altında ayrı blok: "Zorunlu değil ama alıcınız isteyebilir — Kademe B" (14 sektör). Çerçeve §4.3A hibrit kararına bağlı: Kademe B çıktısı asla "SKDM raporu" olarak adlandırılmaz; ISO 14067 mantığında tedarikçi veri dosyasıdır.
2. **Geri dön butonu:** Sihirbaz G-24 kapsamında zaten var; kural genişletildi — tüm modül/içerik sayfalarında görünür "← Geri" (tarayıcı geçmişi güvenli).
3. **CTA akışı:** Ana sayfa birincil CTA'sı sihirbazı doğrudan başlatır (doğru mimari); ancak ana sayfa 5 saniye testini geçmek zorunda — ne (SKDM karbon raporu), kime (AB'ye ihraç eden Türk üretici), nasıl (7 adım, yardımsız), kaça (9.900 ₺ tek seferlik).
4. **İkon dili:** cimpactpro tarzı çizgi ikonlar (Lucide); emoji yasak korunur; yalnız ✓/!/✕ durum glifleri.
5. **/rehber/ ve /sozluk/ sayfaları:** İçerik Kimi tarafından yazıldı (sayfa-icerik-rehber.md, sayfa-icerik-sozluk.md); sözlük tanımları FieldHelp `(?)` panellerinin tek kaynağıdır (G-21).

## Ek E — Sıralama Kararı (16.08.2026, kullanıcı direktifi)

Paddle başvurusu **son kapıdır**: sistem %100 tamamlanmadan başvuru yapılmaz. Sıra: (1) sihirbaz katmanları %100, (2) mühürlü paket/ZIP (Plan 20), (3) kalite kapıları (E2E, erişilebilirlik, performans), (4) Paddle. Paddle'ın 5 gerçek verisi bu süre içinde hazırlanır ama başvuru sona saklanır.
...
## Ek G — Blueprint Değerlendirmesinin Kabulü (16.08.2026, hakem kararı)

Bağımsız inceleme dokümanı (BLUEPRINT_DEGERLENDIRME_VE_EKSIK_KATMANLAR.md) hakemce mevzuata karşı doğrulandı; aşağıdaki hükümler kabul edilerek ana plana işlendi. AGENTS.md kaynak hiyerarşisi ve bu ek birlikte bağlayıcıdır.

### G.1 Mimari düzeltmeler (KABUL — RM/AGENTS çelişen maddelerini geçersiz kılar)

1. **Canonical veri modeli = IR 2025/2547 üçlüsü.** Kesin dönemin hukuki çekirdeği Monitoring Plan + Operator's Emissions Report + Summary'dir. Communication Template (Aralık 2024 sürümü) canonical model değil, bir **çıktı adaptörüdür** (workbook adapter). AB şablonu güncellendiğinde yalnız adapter katmanı güncellenir; veri modeli tüzükten türer. AGENTS.md §1'deki "RM-004 tek doğruluk kaynağı" hükmü buna göre okunur: RM-004 alan şeması IR 2025/2547 alanlarına indirgenir.
2. **CN kod sayısı sabiti yasak.** "569 CN kodu" hiçbir yerde (pazarlama, kod, AGENTS.md) kullanılmaz. Hukuki kapsam Annex I'deki 8 haneli CN kodlarıdır (~250 kod). Kapsam listesi **versioned ruleset** olarak tutulur. AGENTS.md §2 ilk maddesindeki "569 CN kodu" ifadesi geçersizdir.
3. **Endirekt emisyon kuralı CN seviyesinde:** `cnCode → annexIIOnlyDirect`. Annex II yalnız direkt emisyonu fiyatlanan sektörleri tanımlar (demir-çelik, alüminyum, elektrik, hidrojen); çimento ve gübre hem direkt hem endirekt fiyatlanır. Sektör-seviyesi switch yasak.
4. **"Bir CN = bir üretim süreci" katı kuralı yumuşatıldı:** IR 2025/2547 Md.4 functional unit mantığında, aynı functional unit'e sahip malların farklı üretim rotaları tek production process altında toplanabilir; multifunctional process ve joint precursor kuralları uygulanır. AGENTS.md §2 ilgili maddesi buna göre güncellenir.
5. **Doğrulayıcı danışmanlık yasağı daraltıldı:** Doğrulayıcı aynı operatör için izleme/raporlama metodolojisi geliştiren danışmanlık veremez; ama soru sorabilir, bulgu bildirebilir, iyileştirme tavsiyesi verebilir. Pazarlama ifadesi: "Doğrulayıcınız metodolojinizi kurmanızda yardım edemez; bulguyu nasıl kapatacağınızı tasarlayamaz. SKDMHesapla tam bu boşluğu doldurur."
6. **"11 dosya" ürün standardıdır, AB zorunluluğu değil.** Tüm paket/alan içerikleri etiketlenir: [AB-ZORUNLU] / [KOŞULLU] / [ÜRÜN-STANDARDI] / [TİCARİ] / [VARSAYIM] / [BLOCKER].
7. **Kırmızı/hata yasağı WCAG'a göre revize:** Renk tek sinyal olamaz; hata metinle tanımlanır ve düzeltme yolu verilir. Kullanıcıyı suçlamayan dil korunur (Error Copy Dictionary deseni; ör. "Bu alanda ton bekleniyor; 128.450 kg girdiyseniz 128,45 yazın.").
8. **Paket yapısı = 3 view + değişmez çekirdek + versiyonlu manifest.** Importer Pack / Verifier Pack / Operator Archive; her view'ın sabit minimum dosya seti vardır, üstüne koşullu dosyalar eklenir. Manifest versiyonludur (alıcı "v2 paketi" diyebilir).
9. **Confidentiality Engine:** Ölçüm belgeleri, laboratuvar sonuçları, tedarikçi fiyatları doğrulayıcıya gider, ALICIYA GİTMEZ. Verifier/Importer paket ayrımı zorunlu.
10. **YTD vs Final ayrımı:** Yıl kapanmadan "final annual" kilitlenemez; ara dönem çıktıları YTD etiketi taşır.
11. **Revision/correction model + version fork:** Doğrulayıcı düzeltmesi eski mühürlü dosyayı bozmadan yeni sürümde çalışılır.
12. **Heat / Waste gas modülleri** veri modeline eklenir (demir-çelik ve çimento için zorunlu).
13. **"Verifier Challenge Preview" Türkçe UI adı:** "Doğrulayıcı size ne soracak?"

### G.2 Türkiye katmanları (KABUL — stratejik)

14. **TR-ETS çift kullanımlı veri temeli:** Veri modeli bugünden TR-ETS MRV alanlarını taşır; aynı alanlar iki çıktıya map edilir (Seviye 1 hemen). Seviye 2 (TR-ETS İzleme Planı taslağı çıktısı) Faz 2-3. Seviye 3 (2028+ mali dönem) stratejik yol haritası.
15. **TR-ETS mahsup alanı açıklaması (hemen):** Alanın altında sabit metin: "Şu an bu alan sizin için 0'dır. Türkiye ETS'si 2026-2027 pilot döneminde ve bu dönemde tesislere %100 ücretsiz tahsisat veriliyor, mali yükümlülük uygulanmıyor..."
16. **Rota bazlı actual-vs-default dürüst yön tahmini:** EAF seçilirse "gerçek veri genellikle varsayılandan avantajlı çıkar — tesise özgü hesaba bağlıdır"; BF-BOF seçilirse "gerçek veri varsayılanın üzerinde de çıkabilir; iki senaryoyu karşılaştırmalı gösteriyoruz". "Gerçek veri her zaman ucuzdur" ifadesi YASAK.
17. **EAF hurda precursor sadeleştirmesi:** EAF rotasında Precursor modülü sadeleşir + bilgi metni: "Hurda SKDM kapsamında değildir (CN 7204); yalnızca ferro-alaşımlar ve DRI/HBI gibi kapsamdaki girdileri kaydedeceksiniz."
18. **Satış mesajı ekseni:** "Türk çeliği zaten dünyanın en temiz çeliklerinden biri. Bunu kanıtlayamazsanız, kirli üretim için hesaplanmış varsayılan değerle fiyatlanırsınız." (Hak edilmiş avantajı kanıtlama — kayıp kaçınma değil.)
19. **Kayıt saklama takvimi** (Document Control; süre mevzuattan doğrulanacak), **doğrulayıcı kapasite uyarısı** (Faz A'da "doğrulayıcıyla temas?" sorusu + Q4 2026 otomatik uyarı), **alıcı takviminden geriye planlama** (Faz A'da "alıcınız veriyi ne zaman istedi?" → geriye doğru tarih zinciri), **alıcı bazlı format tercihi**, **sayı girişinde yazıyla teyit** (128450 → "yüz yirmi sekiz bin dört yüz elli kWh"), **dosya devir/rol transferi** — hepsi kabul; etiketleriyle birlikte yol haritasına işlendi.

### G.3 Kullanıcı direktifleri (bu tur)

20. **GTİP bilmeyen kullanıcı akışı:** /basla/ ve sihirbaz başında ürün arama kutusu; kullanıcı ürününü yazar ("inşaat demiri", "külçe alüminyum"...), sistem CN kodu önerir + güven notu gösterir ("kod teyidi alıcınızla yapılmalı").
21. **Rehber ve sözlükte arama:** Sayfa üstünde arama kutusu; yazarken filtreleme, sonuç başlıklarına atlama. Sözlükte terim + EN karşılık + içerik üzerinde arama.
22. **Sözlük devasa genişleme v3:** Tüm SKDM/CBAM İngilizce terimlerinin Türkçe karşılığı + anlamı + nerede kullanıldığı; durum bazlı yönlendirici bölümler. Tek kaynak G-21 ilkesi korunur.

---

## Ek H — Plan 26 Dağıtım Konsolidasyonu ve İcra Odakları (16.08.2026)

Plan 26 ile dağıtım altyapısı, kalite testleri ve Ek G kararlarının icra sıralaması konsolide edilmiştir.

### H.1 Dağıtım ve İzleme Standardı (KABUL)
1. **Canlı Sayaç Motoru (`scripts/deploy-with-timer.mjs`):** Dağıtım ve test adımları saniye saniye canlı spinner ve kronometre ile terminalde izlenir (`npm run deploy:live`).
2. **Kilitli Dağıtım Zinciri:** Tip kontrolü (typecheck) ➔ Motor testleri (test:skdm 7/7) ➔ Next.js derlemesi (build) ➔ Firebase hosting & functions dağıtımı ➔ Canlı kalite kapıları (test:quality).

### H.2 Uygulama Öncelikleri
1. **Sözlük v3 & Arama Entegrasyonu:** Sözlük sayfasında dinamik Türkçe/İngilizce terim arama kutusu ve durum bazlı rehber yönlendirmeleri.
2. **Rehber Arama Motoru:** 10 katman ve genel SKDM konularında filtrelemeli anlık başlık/içerik araması.
3. **Ürün/GTİP Eşleştirme Akışı:** Kullanıcı dostu ürün arama ve güven notlu CN kodu önerici.
4. **Türkiye Katmanları (TR-ETS & EAF/BF-BOF Dürüst Bilgilendirme):** Mahsup ve hurda açıklamaları.
